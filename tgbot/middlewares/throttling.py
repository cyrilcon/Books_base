...
class ThrottlingMiddleware(BaseMiddleware):
    def __init__(self, storage: RedisStorage, time_throttling: int, throttled_commands: List[str]):
        self.storage = storage  # Объект хранилища Redis
        self.time_throttling = time_throttling  # Время троттлинла
        self.throttled_commands = throttled_commands  # Команды и фразы, которые нужно троттлить
    ...
    async def __call__(self,
        ...
        if isinstance(event, Message) and event.text in self.throttled_commands:
            ...
            if check_user:
                time_left = await self.storage.redis.ttl(name=user)  # Оставшееся время жизни ключа
                if int(check_user.decode()) == 1:  # Попадает под спам фильтр
                    await self.storage.redis.set(name=user, value=0, ex=time_left)
                    return await event.answer(f"Слишком частый запрос, повторите через <b>{time_left}</b> секунд")
                return
