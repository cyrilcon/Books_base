...
give-book-error-user-already-has-this-book = У пользователя уже есть книга "<code>{ $book_title }</code>" (<code>{ $article }</code>)!!

give-book-given =
    Вы получили подарок от нашего магазина { -books-base }!!
    Книга доступна в вашей личной книжной полке — /my_books

give-book-success = Пользователь { $user_link } (<code>{ $id_user }</code>) получил книгу "<code>{ $book_title }</code>" (<code>{ $article }</code>)
